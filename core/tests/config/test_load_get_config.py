...
from linuxforhealth.healthos.core.config import load_core_configuration, CoreServiceConfig, get_core_configuration
...
def test_load_get_core_configuration(resources_path: str, monkeypatch):
    ...
    """
    monkeypatch.setattr(linuxforhealth.healthos.core.config, "core_service_config", None)
    config: CoreServiceConfig = get_core_configuration()
    assert config is None

    config_path: str = os.path.join(resources_path, "service-config", "core-service.yml")
